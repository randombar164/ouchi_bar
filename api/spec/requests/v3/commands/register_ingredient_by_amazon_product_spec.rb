...
RSpec.describe 'V3::Commands::RegisterIngredientByAmazonProduct', type: :request do
  let!(:amazon_browse_node) { V3::AmazonBrowseNode.create!(name: "amazon_browse_node1", amazon_browse_node_id: 1000, node_path: "hogehoge") }
  ...
  describe 'POST v3/commands/register_ingredient_by_amazon_product' do
    it 'responds successfully returns a 200 response' do
      allow_any_instance_of(SearchAmazonProductsService).to receive(:call).and_return([{'ASIN' => 'asin', 'DetailPageURL' => 'detail_page_url', 'BrowseNodeInfo' => {'BrowseNodes' => [{'Id' => amazon_browse_node.amazon_browse_node_id}]}, 'Images' => {'Primary' => {'Medium' => {'URL' => 'image_url'}}}, 'ItemInfo' => {'Title' => {'DisplayValue' => 'item_name'}}}])
